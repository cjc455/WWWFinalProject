  ...
  $(window).scroll(function (event) {
    ...
    //It adds up all previous moves, as well as applies the current move to get the final position.
      ...
      for(i = 0; i < scroll_moves.length; i++){
        ...
        if(i == scroll_moves.length - 1 && current_move.endY <= scroll) {
          end_of_page();

        }
        if (scroll > current_move.startY){
            scroll_ammount = scroll - current_move.startY;
        }
        if(scroll > current_move.endY) {
            scroll_ammount = current_move.endY - current_move.startY;
        }
        var j = 0;
        for(j = 0; j < scroll_objects.length; j++){
